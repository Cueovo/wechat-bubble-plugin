...
9. **Liquid Glass 路由：iOS 26 使用动态验证后的 `UIGlassEffect`；旧系统进入聊天时优先隔离大型非消息 `UIImageView` 壁纸分支并捕获一次共享 IOSurface-backed 背景，使用红绿区域扫描校准纹理 Y 方向，随后所有可见气泡按实时窗口坐标逐帧执行 Metal SDF 折射、RGB 色散、轻微放大、明暗自适应和方向性边缘高光；窗口尺寸或聊天生命周期变化时使缓存失效，Metal 或捕获不可用时回退无色 UIKit 镜片。**
