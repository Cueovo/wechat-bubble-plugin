...
- 决策日期：2026-08-03
- 文档状态：阶段 01/02/03/04 已完成；0.3.3 已通过插件归纳入口和设置数值调整真机验收；阶段 05 的 0.4.0 启动哨兵已构建并通过正常启动检查、故障演练待完成；0.5.1 无色玻璃和微信素材隔离已构建，0.6.4 已证明当前系统的私有 `displacementMap` 虽接受全部参数但不产生可见位移，0.6.5 已切换到显式局部背景捕获和 Core Image warp 并待真机复测；阶段 06 尚未完成
- 项目阶段：MVP
...
9. **Liquid Glass 路由：iOS 26 使用动态验证后的 `UIGlassEffect`；旧系统显式捕获气泡后的局部窗口内容，并用公开 Core Image warp 按圆角 SDF 边缘重采样；捕获或 warp 不可用时回退无色 UIKit 镜片。**
