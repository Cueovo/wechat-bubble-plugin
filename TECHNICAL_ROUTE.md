...
- 决策日期：2026-08-03
- 文档状态：阶段 01/02/03/04 已完成；0.3.3 已通过插件归纳入口和设置数值调整真机验收；阶段 05 的 0.4.0 启动哨兵已构建并通过正常启动检查、故障演练待完成；0.5.1 无色玻璃和微信素材隔离已构建，0.6.3 正在修正旧系统 SDF 位移图 backing scale 并保留安全回退；阶段 06 尚未完成
- 项目阶段：MVP
...
9. **Liquid Glass 路由：iOS 26 使用动态验证后的 `UIGlassEffect`；旧系统优先使用 `CABackdropLayer + CAFilter(displacementMap)` 的缓存 SDF 位移图，私有能力缺失或运行时异常时回退无色 UIKit 镜片。**
