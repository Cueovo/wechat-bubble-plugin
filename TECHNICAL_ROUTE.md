...
- 决策日期：2026-08-03
- 文档状态：阶段 01/02/03/04 已完成；0.3.3 已通过插件归纳入口和设置数值调整真机验收；阶段 05 的 0.4.0 启动哨兵已构建并通过正常启动检查、故障演练待完成；0.5.1 无色玻璃和微信素材隔离已构建，0.6.4 已证明当前系统的私有 `displacementMap` 虽接受全部参数但不产生可见位移，0.6.5 因停止滚动后才更新而否决，0.7.0 的每帧全窗口捕获因 51ms 主线程成本和纹理坐标错位被真机否决，0.7.1 已改为自动纹理方向校准、一次干净背景捕获和逐帧实时 UV 采样并待复测；阶段 06 尚未完成
- 项目阶段：MVP
...
9. **Liquid Glass 路由：iOS 26 使用动态验证后的 `UIGlassEffect`；旧系统进入聊天时捕获一次排除文字 Cell 的干净窗口背景到 IOSurface-backed `CVPixelBuffer`，自动校准纹理 Y 方向，随后所有可见气泡共享该 Metal 纹理并按实时窗口坐标逐帧执行 SDF 折射、RGB 色散、轻微放大、明暗自适应和方向性边缘高光；窗口尺寸或聊天生命周期变化时使缓存失效，Metal 或捕获不可用时回退无色 UIKit 镜片。**
