...
9. **Liquid Glass 路由：iOS 26 使用动态验证后的 `UIGlassEffect`；旧系统隔离聊天壁纸到共享 IOSurface-backed 背景纹理，按实时窗口坐标采样；显示链路仅在滚动、布局、外观或生命周期变化时运行，稳定后暂停；每个真实 `UIBezierPath` 生成并缓存 R8 单通道 SDF，主体、尾巴和消息分段共用同一距离场计算 alpha、法线、折射、色散、高光和暗边，SDF 或 Metal 不可用时保留圆角近似和无色 UIKit 镜片回退。**
