...
| 04 | [设置与持久化](./04-preferences.md) | 微信内设置、深浅色主题、材质切换和恢复默认 | 基础与 0.5.1 已完成；0.6.x 私有滤镜和停止后快照路线均已否决，0.7.0 每帧全窗口捕获因 51ms 卡顿和坐标错位否决，0.7.1 消除逐帧捕获但方向回退采到输入栏控件，0.7.2 壁纸隔离与区域校准路线待复测 |
| 05 | [安全与兼容](./05-safety-compatibility.md) | 安全模式、版本门控、诊断和性能达标 | 进行中（0.4.0 启动哨兵待真机故障演练） |
