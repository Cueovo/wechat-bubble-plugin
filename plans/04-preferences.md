...
实现状态：按需显示链路已保留；首版尺寸自适应光学因真机折射弱于 0.7.2，已在 0.8.0.1 回退到验证过的固定光学基线。
...
- 首版曾使用气泡短边缩放 `edgeWidth`、`refraction`、`dispersion` 和 `zoom`，但真机确认折射明显弱于 0.7.2；0.8.0.1 恢复上一版固定参数，后续自适应必须以不降低中等气泡折射强度为前提重新标定。
...
真机反馈确认 0.8.0 存在三项回归：自适应参数令折射弱于上一版；发送消息时路径连续布局会在 UIKit fallback 与 Metal 之间反复切换；新轮廓的 SDF 在主线程同步生成，并且 Shader 每像素读取五次 SDF，连续多条消息滚动时产生卡顿感。

- 恢复 0.7.2 已验证的 `edgeWidth=15`、`refraction=13`、`dispersion=1.6`、`zoom=2.2` 光学基线，并扩大 SDF 距离编码范围，避免强折射区被过早截断。
- 已成功输出过 Metal 帧的 renderer 在同一窗口内发生尺寸、路径或 appearance 更新时继续保留最后一帧，不再重新启用 UIKit blur；generation 仍会递增以拒绝过期 command buffer。
- SDF 缓存未命中时改由串行 utility queue 生成；主线程只计算轻量路径签名、读取缓存和接收完成结果，相同 renderer 的重复 layout 不会重复排队。
- Shader 使用单次 SDF 采样配合屏幕空间导数计算法线，将 SDF 读取从每像素五次降到一次；真实路径 alpha、法线与圆角回退仍保留。
- 诊断格式升级到 23，增加异步 SDF 请求/完成计数、生成策略和每片元 SDF 采样数。
