...
| 原生 RootHide 包安装 | 待验证 | |
| 微信冷启动 | 0.0.2/0.0.3/0.0.4 已通过 | |
| 前后台切换 | 待验证 | |
| `diagnostics.plist` 生成 | 0.0.2/0.0.3/0.0.4 已通过 | |
| 主可执行文件 | 已确认 | `WeChat` |
| `CFBundleVersion` | 已确认 | `8.0.60.35` |
| 候选类快照 | 已确认 | 四个候选类均存在 |
| 匿名布局观察样本 | 已确认 | 0.0.3：44 次布局、16 个去重样本 |
| 左右方向判定 | 已确认 | 0.0.4：24 个样本均由头像判定，left 15 / right 9；稳定 x 约为 9 / 375 |
| 背景呈现来源 | 已确认 | `YYAsyncImageView.layer.contents` 异步出现，普通/动画图片 getter 均无图像 |
| 卸载后微信正常 | 待验证 | |
